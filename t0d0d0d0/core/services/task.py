...
from t0d0d0d0.core.infra.db.models import TaskModel, NewTaskModel, NameTaskModel
...
class TaskService: 
    ...
    async def newTask(self, user_id:int, data:NewTaskSch) -> TaskModel:
        """required: database"""
        async with self.uow:
            ...
            return TaskModel(**t.model().model_dump())
    ...
    async def getTasks(self, user_id:int) -> list[TaskModel]:
        """required: database"""
        async with self.uow:
            u = await self.uow.user.get_one(id=user_id)
            if not u: raise AuthException('User not found')
            t = await self.uow.task.get_by_date(user_id=user_id)
            r = [NameTaskModel(**i.model().model_dump(), project_name=i.project.name) if i.project else NameTaskModel(**i.model().model_dump(), project_name=None) for i in t]
            return r
        
    async def getTaskByDate(self, user_id:int, date:datetype) -> list[TaskModel]:
        """required: database"""
        async with self.uow:
            u = await self.uow.user.get_one(id=user_id)
            if not u: raise AuthException('User not found')
